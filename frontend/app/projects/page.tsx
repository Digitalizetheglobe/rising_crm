  ...
  const handleDeleteProject = async (projectId: string, projectName: string) => {
    ...
    try {
      ...
      if (json.success) {
        ...
        // State and counters refresh karo
        fetchProjects();
        fetchStats();
        fetchLocations();
      } else {
        addToast(json.message || "Failed to delete project", "info");
      }
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : "Error connecting to server";
      addToast(message, "info");
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {projects.map((project) => (
            ...
            >
              ...
              {/* Layout Modified: Side-by-Side View & Delete Button */}
              <div className="flex items-center gap-3 w-full">
                <button
                  onClick={() => handleOpenDetails(project.id)}
                  className="flex-1 bg-[#22c55e] hover:bg-[#16a34a] text-white font-bold text-[14px] py-3 rounded-2xl transition-colors cursor-pointer"
                >
                  View Details
                </button>
                <button
                  ...
                  className="bg-red-50 hover:bg-red-100 text-red-600 p-3.5 rounded-2xl transition-colors border border-red-100 cursor-pointer flex items-center justify-center"
                >
